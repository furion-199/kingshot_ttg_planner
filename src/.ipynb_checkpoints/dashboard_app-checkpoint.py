...
def _validation_panel_simple(start_tg_level: int, target_tg_level: int, scenario: str, building_tg_cost: int, target_ttg: int) -> pd.DataFrame:
    rows = [{
        "mode": "simple",
        "scenario": scenario,
        "start_tg_level": start_tg_level,
        "target_tg_level": target_tg_level,
        "building_cost_tg": building_tg_cost,
        "target_ttg": target_ttg,
    }]
...
for b in BUILDINGS:
    st.session_state.setdefault(f"{b}_start", 5)
    st.session_state.setdefault(f"{b}_target", 7)
...
with st.sidebar:
    ...
    if advanced_mode:
        ...
        adv_all_start = c_a.selectbox("Set all starts", TG_LEVELS, index=0, key="adv_all_start")
        adv_all_target = c_b.selectbox("Set all targets", TG_LEVELS, index=2, key="adv_all_target")
        if c_c.button("Apply to all buildings"):
            for b in BUILDINGS:
                st.session_state[f"{b}_start"] = adv_all_start
                st.session_state[f"{b}_target"] = adv_all_target
        if st.button("Copy simple mode values into advanced"):
            for b in BUILDINGS:
                st.session_state[f"{b}_start"] = st.session_state["simple_start_tg"]
                st.session_state[f"{b}_target"] = st.session_state["simple_target_tg"]
        if st.button("Reset advanced to TG5 → TG7"):
            for b in BUILDINGS:
                st.session_state[f"{b}_start"] = 5
                st.session_state[f"{b}_target"] = 7
        ...
        for b in BUILDINGS:
            c1, c2 = st.columns(2)
            start_b = c1.selectbox(f"{b} start", TG_LEVELS, key=f"{b}_start")
            target_b = c2.selectbox(f"{b} target", TG_LEVELS, key=f"{b}_target")
            if target_b < start_b:
                advanced_valid = False
            building_inputs[b] = {"start": start_b, "target": target_b}
            adv_rows.append({"Building": b, "Start": start_b, "Target": target_b, "Included": target_b > start_b})
        ...
        if not advanced_valid:
            ...
            try:
                ...
                totals = _lookup_totals_advanced(start_levels=start_levels, target_levels=target_levels, default_start_level=5)
        ...
        st.caption("Advanced mode ignores the scenario dropdown and includes any building with target > start.")
...
use_advanced = planning_mode == "advanced"
...
with tab6:
    ...
    if planning_mode is None:
        ...
        val_df = _validation_panel_simple(start_tg_level, target_tg_level, scenario, target_tg, target_ttg)
        st.dataframe(val_df, use_container_width=True, hide_index=True)
